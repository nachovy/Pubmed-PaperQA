...
def fetch(pmid, finders, name, headers, errorPmids, output_dir):
    ...
    if (os.path.exists("{0}/{1}.pdf".format(output_dir, pmid)) or
        os.path.exists("{0}/{1}.html".format(output_dir, pmid))):  # bypass finders if pdf reprint already stored locally
